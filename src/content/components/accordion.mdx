---
title: Accordion
description: Expandable sections that reveal content one at a time.
category: disclosure
tagline: Expandable content sections
status: stable
order: 1
---

import HugeIcon from "@/components/ui/icon/HugeIcon.astro";
import { Notification01Icon, Clock01Icon, CreditCardIcon, Mail01Icon, Location01Icon, MinusSignIcon, Call02Icon, PlusSignIcon, Settings01Icon, Shield01Icon, UserIcon } from "@/components/ui/icon/icons";

import ComponentPreview from "@/components/docs/ComponentPreview.astro";
import CodeBlock from "@/components/docs/CodeBlock.astro";
import Prose from "@/components/docs/Prose.astro";
import Accordion from "@/components/ui/accordion/Accordion.astro";
import AccordionDescription from "@/components/ui/accordion/AccordionDescription.astro";
import AccordionItem from "@/components/ui/accordion/AccordionItem.astro";
import AccordionLabel from "@/components/ui/accordion/AccordionLabel.astro";
import AccordionTrigger from "@/components/ui/accordion/AccordionTrigger.astro";
import AccordionContent from "@/components/ui/accordion/AccordionContent.astro";
import Badge from "@/components/ui/badge/Badge.astro";
import Button from "@/components/ui/button/Button.astro";











<ComponentPreview id="single">
  <Fragment slot="code">
```html
<Accordion type="single">
  <AccordionItem value="item-1">
    <AccordionTrigger>Is it accessible?</AccordionTrigger>
    <AccordionContent>Yes. It adheres to the WAI-ARIA design pattern.</AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-2">
    <AccordionTrigger>Is it styled?</AccordionTrigger>
    <AccordionContent>Yes. It comes with default styles that match your theme.</AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-3">
    <AccordionTrigger>Is it animated?</AccordionTrigger>
    <AccordionContent>Yes. It uses CSS animations for smooth transitions.</AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full">
    <AccordionItem value="item-1">
      <AccordionTrigger>Is it accessible?</AccordionTrigger>
      <AccordionContent>Yes. It adheres to the WAI-ARIA design pattern.</AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-2">
      <AccordionTrigger>Is it styled?</AccordionTrigger>
      <AccordionContent>Yes. It comes with default styles that match your theme.</AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-3">
      <AccordionTrigger>Is it animated?</AccordionTrigger>
      <AccordionContent>Yes. It uses CSS animations for smooth transitions.</AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>

<div class="space-y-4">
<Prose>
### Installation
</Prose>

<ComponentPreview component="accordion" />
</div>

<div class="space-y-4">
<Prose>
### Usage

```ts
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  AccordionTrigger,
} from "@/components/bearnie/accordion"
```
```html
<Accordion type="single">
  <AccordionItem value="item-1">
    <AccordionTrigger>Is it accessible?</AccordionTrigger>
    <AccordionContent>
      Yes. It adheres to the WAI-ARIA design pattern.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```

</Prose>
</div>

<div class="space-y-8">
<Prose>
### Examples
</Prose>

<div class="space-y-4">
<Prose>
#### Multiple

Multiple items can be open simultaneously. Useful when users may need to compare content across sections.
</Prose>

<ComponentPreview id="multiple">
  <Fragment slot="code">
```html
<Accordion type="multiple">
  <AccordionItem value="item-1">
    <AccordionTrigger>Personal Information</AccordionTrigger>
    <AccordionContent>Name, email, and Call02Icon number details.</AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-2">
    <AccordionTrigger>Billing Address</AccordionTrigger>
    <AccordionContent>Street address, city, state, and zip code.</AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-3">
    <AccordionTrigger>Payment Method</AccordionTrigger>
    <AccordionContent>Credit card or PayPal account information.</AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="multiple" class="w-full">
    <AccordionItem value="item-1">
      <AccordionTrigger>Personal Information</AccordionTrigger>
      <AccordionContent>Name, email, and Call02Icon number details.</AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-2">
      <AccordionTrigger>Billing Address</AccordionTrigger>
      <AccordionContent>Street address, city, state, and zip code.</AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-3">
      <AccordionTrigger>Payment Method</AccordionTrigger>
      <AccordionContent>Credit card or PayPal account information.</AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### With Icon

Add icons to accordion triggers to provide visual context for each section.
</Prose>

<ComponentPreview id="with-icon">
  <Fragment slot="code">
```html
<Accordion type="single">
  <AccordionItem value="profile">
    <AccordionTrigger>
      <span class="flex items-center gap-2">
        <HugeIcon icon={UserIcon} class="size-4" />
        Profile Settings
      </span>
    </AccordionTrigger>
    <AccordionContent>
      Manage your profile information, avatar, and display name.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="billing">
    <AccordionTrigger>
      <span class="flex items-center gap-2">
        <HugeIcon icon={CreditCardIcon} class="size-4" />
        Billing
      </span>
    </AccordionTrigger>
    <AccordionContent>
      View and manage your billing information and payment methods.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="notifications">
    <AccordionTrigger>
      <span class="flex items-center gap-2">
        <HugeIcon icon={Notification01Icon} class="size-4" />
        Notifications
      </span>
    </AccordionTrigger>
    <AccordionContent>
      Configure how and when you receive notifications.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full">
    <AccordionItem value="profile">
      <AccordionTrigger>
        <span class="flex items-center gap-2">
          <HugeIcon icon={UserIcon} class="size-4" />
          Profile Settings
        </span>
      </AccordionTrigger>
      <AccordionContent>
        Manage your profile information, avatar, and display name.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="billing">
      <AccordionTrigger>
        <span class="flex items-center gap-2">
          <HugeIcon icon={CreditCardIcon} class="size-4" />
          Billing
        </span>
      </AccordionTrigger>
      <AccordionContent>
        View and manage your billing information and payment methods.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="notifications">
      <AccordionTrigger>
        <span class="flex items-center gap-2">
          <HugeIcon icon={Notification01Icon} class="size-4" />
          Notifications
        </span>
      </AccordionTrigger>
      <AccordionContent>
        Configure how and when you receive notifications.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### With Description

Add a subtitle or description below the main trigger text for additional context.

Use `AccordionLabel` for the main trigger text and `AccordionDescription` for the supporting text. The components also provide the appropriate accessible name and description for the trigger.
</Prose>

<ComponentPreview id="with-description">
  <Fragment slot="code">
```html
<Accordion type="single">
  <AccordionItem value="account">
    <AccordionTrigger>
      <div class="flex flex-col items-start gap-1">
        <AccordionLabel>Account</AccordionLabel>
        <AccordionDescription>
          Manage your account settings
        </AccordionDescription>
      </div>
    </AccordionTrigger>
    <AccordionContent>
      Update your username, email, and password.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="privacy">
    <AccordionTrigger>
      <div class="flex flex-col items-start gap-1">
        <AccordionLabel>Privacy</AccordionLabel>
        <AccordionDescription>
          Control your privacy preferences
        </AccordionDescription>
      </div>
    </AccordionTrigger>
    <AccordionContent>
      Manage who can see your profile and Activity01Icon.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="security">
    <AccordionTrigger>
      <div class="flex flex-col items-start gap-1">
        <AccordionLabel>Security</AccordionLabel>
        <AccordionDescription>
          Protect your account
        </AccordionDescription>
      </div>
    </AccordionTrigger>
    <AccordionContent>
      Enable two-factor authentication and manage sessions.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full">
    <AccordionItem value="account">
      <AccordionTrigger>
        <div class="flex flex-col items-start gap-1">
          <AccordionLabel>Account</AccordionLabel>
          <AccordionDescription>
            Manage your account settings
          </AccordionDescription>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        Update your username, email, and password.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="privacy">
      <AccordionTrigger>
        <div class="flex flex-col items-start gap-1">
          <AccordionLabel>Privacy</AccordionLabel>
          <AccordionDescription>
            Control your privacy preferences
          </AccordionDescription>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        Manage who can see your profile and Activity01Icon.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="security">
      <AccordionTrigger>
        <div class="flex flex-col items-start gap-1">
          <AccordionLabel>Security</AccordionLabel>
          <AccordionDescription>
            Protect your account
          </AccordionDescription>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        Enable two-factor authentication and manage sessions.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### With Icon and Description

Combine icons with descriptions for a rich, informative accordion header.
</Prose>

<ComponentPreview id="with-icon-description">
  <Fragment slot="code">
```html
<Accordion type="single">
  <AccordionItem value="profile">
    <AccordionTrigger>
      <div class="flex items-start gap-3">
        <HugeIcon icon={UserIcon} class="size-5 mt-0.5 text-muted-foreground" />
        <div class="flex flex-col items-start gap-1">
          <AccordionLabel>Profile</AccordionLabel>
          <AccordionDescription>
            Your personal information
          </AccordionDescription>
        </div>
      </div>
    </AccordionTrigger>
    <AccordionContent>
      Update your name, bio, and profile picture.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="security">
    <AccordionTrigger>
      <div class="flex items-start gap-3">
        <HugeIcon icon={Shield01Icon} class="size-5 mt-0.5 text-muted-foreground" />
        <div class="flex flex-col items-start gap-1">
          <AccordionLabel>Security</AccordionLabel>
          <AccordionDescription>
            Password and authentication
          </AccordionDescription>
        </div>
      </div>
    </AccordionTrigger>
    <AccordionContent>
      Change your password and enable 2FA.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="notifications">
    <AccordionTrigger>
      <div class="flex items-start gap-3">
        <HugeIcon icon={Notification01Icon} class="size-5 mt-0.5 text-muted-foreground" />
        <div class="flex flex-col items-start gap-1">
          <AccordionLabel>Notifications</AccordionLabel>
          <AccordionDescription>
            Email and push alerts
          </AccordionDescription>
        </div>
      </div>
    </AccordionTrigger>
    <AccordionContent>
      Choose what notifications you want to receive.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full">
    <AccordionItem value="profile">
      <AccordionTrigger>
        <div class="flex items-start gap-3">
          <HugeIcon icon={UserIcon} class="size-5 mt-0.5 text-muted-foreground" />
          <div class="flex flex-col items-start gap-1">
            <AccordionLabel>Profile</AccordionLabel>
            <AccordionDescription>
              Your personal information
            </AccordionDescription>
          </div>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        Update your name, bio, and profile picture.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="security">
      <AccordionTrigger>
        <div class="flex items-start gap-3">
          <HugeIcon icon={Shield01Icon} class="size-5 mt-0.5 text-muted-foreground" />
          <div class="flex flex-col items-start gap-1">
            <AccordionLabel>Security</AccordionLabel>
            <AccordionDescription>
              Password and authentication
            </AccordionDescription>
          </div>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        Change your password and enable 2FA.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="notifications">
      <AccordionTrigger>
        <div class="flex items-start gap-3">
          <HugeIcon icon={Notification01Icon} class="size-5 mt-0.5 text-muted-foreground" />
          <div class="flex flex-col items-start gap-1">
            <AccordionLabel>Notifications</AccordionLabel>
            <AccordionDescription>
              Email and push alerts
            </AccordionDescription>
          </div>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        Choose what notifications you want to receive.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Nested Accordions

Accordions can be nested inside each other for hierarchical content organization.
</Prose>

<ComponentPreview id="nested">
  <Fragment slot="code">
```html
<Accordion type="single">
  <AccordionItem value="general">
    <AccordionTrigger>
      <span class="flex items-center gap-2">
        <HugeIcon icon={Settings01Icon} class="size-4" />
        General Settings
      </span>
    </AccordionTrigger>
    <AccordionContent>
      <Accordion type="single" class="border-l pl-4">
        <AccordionItem value="language">
          <AccordionTrigger>Language</AccordionTrigger>
          <AccordionContent>
            Select your preferred language for the interface.
          </AccordionContent>
        </AccordionItem>
        <AccordionItem value="timezone">
          <AccordionTrigger>Timezone</AccordionTrigger>
          <AccordionContent>
            Set your local timezone for accurate timestamps.
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="notifications">
    <AccordionTrigger>
      <span class="flex items-center gap-2">
        <HugeIcon icon={Notification01Icon} class="size-4" />
        Notification Preferences
      </span>
    </AccordionTrigger>
    <AccordionContent>
      <Accordion type="single" class="border-l pl-4">
        <AccordionItem value="email">
          <AccordionTrigger>Email Notifications</AccordionTrigger>
          <AccordionContent>
            Configure which emails you want to receive.
          </AccordionContent>
        </AccordionItem>
        <AccordionItem value="push">
          <AccordionTrigger>Push Notifications</AccordionTrigger>
          <AccordionContent>
            Manage browser and mobile push alerts.
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full">
    <AccordionItem value="general">
      <AccordionTrigger>
        <span class="flex items-center gap-2">
          <HugeIcon icon={Settings01Icon} class="size-4" />
          General Settings
        </span>
      </AccordionTrigger>
      <AccordionContent>
        <Accordion type="single" class="border-l pl-4">
          <AccordionItem value="language">
            <AccordionTrigger>Language</AccordionTrigger>
            <AccordionContent>
              Select your preferred language for the interface.
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="timezone">
            <AccordionTrigger>Timezone</AccordionTrigger>
            <AccordionContent>
              Set your local timezone for accurate timestamps.
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="notifications">
      <AccordionTrigger>
        <span class="flex items-center gap-2">
          <HugeIcon icon={Notification01Icon} class="size-4" />
          Notification Preferences
        </span>
      </AccordionTrigger>
      <AccordionContent>
        <Accordion type="single" class="border-l pl-4">
          <AccordionItem value="email">
            <AccordionTrigger>Email Notifications</AccordionTrigger>
            <AccordionContent>
              Configure which emails you want to receive.
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="push">
            <AccordionTrigger>Push Notifications</AccordionTrigger>
            <AccordionContent>
              Manage browser and mobile push alerts.
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Rich Content

Accordion content can contain any elements including lists, badges, buttons, and more.
</Prose>

<ComponentPreview id="rich-content">
  <Fragment slot="code">
```html
<Accordion type="single">
  <AccordionItem value="contact">
    <AccordionTrigger>Contact Information</AccordionTrigger>
    <AccordionContent>
      <div class="space-y-3">
        <div class="flex items-center gap-2 text-sm">
          <HugeIcon icon={Mail01Icon} class="size-4 text-muted-foreground" />
          <span>support@example.com</span>
        </div>
        <div class="flex items-center gap-2 text-sm">
          <HugeIcon icon={Call02Icon} class="size-4 text-muted-foreground" />
          <span>+1 (555) 123-4567</span>
        </div>
        <div class="flex items-center gap-2 text-sm">
          <HugeIcon icon={Location01Icon} class="size-4 text-muted-foreground" />
          <span>123 Main St, City, Country</span>
        </div>
      </div>
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="hours">
    <AccordionTrigger>Business Hours</AccordionTrigger>
    <AccordionContent>
      <div class="space-y-2">
        <div class="flex items-center justify-between text-sm">
          <span>Monday - Friday</span>
          <Badge variant="outline">9:00 AM - 6:00 PM</Badge>
        </div>
        <div class="flex items-center justify-between text-sm">
          <span>Saturday</span>
          <Badge variant="outline">10:00 AM - 4:00 PM</Badge>
        </div>
        <div class="flex items-center justify-between text-sm">
          <span>Sunday</span>
          <Badge variant="secondary">Closed</Badge>
        </div>
      </div>
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="actions">
    <AccordionTrigger>Quick Actions</AccordionTrigger>
    <AccordionContent>
      <div class="flex flex-wrap gap-2">
        <Button size="sm">Schedule a Call</Button>
        <Button size="sm" variant="outline">Send Message</Button>
        <Button size="sm" variant="secondary">View FAQ</Button>
      </div>
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full">
    <AccordionItem value="contact">
      <AccordionTrigger>Contact Information</AccordionTrigger>
      <AccordionContent>
        <div class="space-y-3">
          <div class="flex items-center gap-2 text-sm">
            <HugeIcon icon={Mail01Icon} class="size-4 text-muted-foreground" />
            <span>support@example.com</span>
          </div>
          <div class="flex items-center gap-2 text-sm">
            <HugeIcon icon={Call02Icon} class="size-4 text-muted-foreground" />
            <span>+1 (555) 123-4567</span>
          </div>
          <div class="flex items-center gap-2 text-sm">
            <HugeIcon icon={Location01Icon} class="size-4 text-muted-foreground" />
            <span>123 Main St, City, Country</span>
          </div>
        </div>
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="hours">
      <AccordionTrigger>Business Hours</AccordionTrigger>
      <AccordionContent>
        <div class="space-y-2">
          <div class="flex items-center justify-between text-sm">
            <span>Monday - Friday</span>
            <Badge variant="outline">9:00 AM - 6:00 PM</Badge>
          </div>
          <div class="flex items-center justify-between text-sm">
            <span>Saturday</span>
            <Badge variant="outline">10:00 AM - 4:00 PM</Badge>
          </div>
          <div class="flex items-center justify-between text-sm">
            <span>Sunday</span>
            <Badge variant="secondary">Closed</Badge>
          </div>
        </div>
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="actions">
      <AccordionTrigger>Quick Actions</AccordionTrigger>
      <AccordionContent>
        <div class="flex flex-wrap gap-2">
          <Button size="sm">Schedule a Call</Button>
          <Button size="sm" variant="outline">Send Message</Button>
          <Button size="sm" variant="secondary">View FAQ</Button>
        </div>
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Borderless

Remove borders for a cleaner, minimal look.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<Accordion type="single">
  <AccordionItem value="item-1" class="border-none">
    <AccordionTrigger>What is your return policy?</AccordionTrigger>
    <AccordionContent>
      We accept returns within 30 days of purchase.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-2" class="border-none">
    <AccordionTrigger>How long does shipping take?</AccordionTrigger>
    <AccordionContent>
      Standard shipping takes 5-7 business days.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-3" class="border-none">
    <AccordionTrigger>Do you ship internationally?</AccordionTrigger>
    <AccordionContent>
      Yes, we ship to over 50 countries worldwide.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full">
    <AccordionItem value="item-1" class="border-none">
      <AccordionTrigger>What is your return policy?</AccordionTrigger>
      <AccordionContent>
        We accept returns within 30 days of purchase.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-2" class="border-none">
      <AccordionTrigger>How long does shipping take?</AccordionTrigger>
      <AccordionContent>
        Standard shipping takes 5-7 business days.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-3" class="border-none">
      <AccordionTrigger>Do you ship internationally?</AccordionTrigger>
      <AccordionContent>
        Yes, we ship to over 50 countries worldwide.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Grouped

Wrap accordion items in a bordered container for a unified, grouped appearance.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<Accordion type="single" class="border rounded-lg">
  <AccordionItem value="item-1" class="border-b last:border-b-0 px-4">
    <AccordionTrigger>First Accordion</AccordionTrigger>
    <AccordionContent>
      Content for the first accordion item.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-2" class="border-b last:border-b-0 px-4">
    <AccordionTrigger>Second Accordion</AccordionTrigger>
    <AccordionContent>
      Content for the second accordion item.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-3" class="border-b last:border-b-0 px-4">
    <AccordionTrigger>Third Accordion</AccordionTrigger>
    <AccordionContent>
      Content for the third accordion item.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full border rounded-lg">
    <AccordionItem value="item-1" class="border-b last:border-b-0 px-4">
      <AccordionTrigger>First Accordion</AccordionTrigger>
      <AccordionContent>
        Content for the first accordion item.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-2" class="border-b last:border-b-0 px-4">
      <AccordionTrigger>Second Accordion</AccordionTrigger>
      <AccordionContent>
        Content for the second accordion item.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-3" class="border-b last:border-b-0 px-4">
      <AccordionTrigger>Third Accordion</AccordionTrigger>
      <AccordionContent>
        Content for the third accordion item.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Card Style

Style accordion items as separate cards with rounded corners and background.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<Accordion type="single" class="space-y-2">
  <AccordionItem value="item-1" class="border rounded-lg px-4">
    <AccordionTrigger>Getting Started</AccordionTrigger>
    <AccordionContent>
      Follow our quick start guide to set up your first project.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-2" class="border rounded-lg px-4">
    <AccordionTrigger>Configuration</AccordionTrigger>
    <AccordionContent>
      Learn how to customize the accordion to fit your needs.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="item-3" class="border rounded-lg px-4">
    <AccordionTrigger>Advanced Usage</AccordionTrigger>
    <AccordionContent>
      Explore advanced features and integrations.
    </AccordionContent>
  </AccordionItem>
</Accordion>
```
  </Fragment>
  <Accordion type="single" class="w-full space-y-2">
    <AccordionItem value="item-1" class="border rounded-lg px-4">
      <AccordionTrigger>Getting Started</AccordionTrigger>
      <AccordionContent>
        Follow our quick start guide to set up your first project.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-2" class="border rounded-lg px-4">
      <AccordionTrigger>Configuration</AccordionTrigger>
      <AccordionContent>
        Learn how to customize the accordion to fit your needs.
      </AccordionContent>
    </AccordionItem>
    <AccordionItem value="item-3" class="border rounded-lg px-4">
      <AccordionTrigger>Advanced Usage</AccordionTrigger>
      <AccordionContent>
        Explore advanced features and integrations.
      </AccordionContent>
    </AccordionItem>
  </Accordion>
</ComponentPreview>
</div>
</div>

<Prose>
### Anatomy

- `Accordion` - The root container that manages state
- `AccordionItem` - Individual collapsible section wrapper
- `AccordionTrigger` - The clickable header that toggles content
- `AccordionLabel` - The primary accessible label for the trigger
- `AccordionDescription` - Supporting accessible description for the trigger
- `AccordionContent` - The expandable content panel
</Prose>

<Prose>
### Props

#### Accordion

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| type | single \| multiple | single | Allow one or multiple items open |
| class | string | - | Additional CSS classes |

#### AccordionItem

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| value | string | required | Unique identifier for the item |
| class | string | - | Additional CSS classes |

#### AccordionTrigger

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| class | string | - | Additional CSS classes |

#### AccordionLabel / AccordionDescription

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| id | string | generated | Optional ID used by the automatic ARIA relationship |
| class | string | - | Additional CSS classes |

#### AccordionContent

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| class | string | - | Additional CSS classes |
</Prose>
